...
| Produtos Compostos | Modelar tipo de produto e composicao | Sem endpoint dedicado nesta etapa | `Produto`, `TipoProduto`, `ProdutoComposicao` | `app/produtos_categorias_test.py`, `app/bootstrap_test.py` | Parcial | Modelagem e regras base implementadas; endpoints e integracao pendentes |
...
| Estoque | Baixa/devolucao automatica por comanda | API de itens de comanda | `Produto`, `ItemComanda`, `MovimentoEstoque` | `app/comandas_test.py` | Implementado | Movimento automatico sem endpoint proprio |
