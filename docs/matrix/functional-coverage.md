...
| Estoque | Consultar consumo fisico por produto | `GET /api/relatorios/estoque-consumido` | `Produto`, `MovimentoEstoque` | `app/relatorios_test.py` | Implementado | Usa movimentos de estoque; saidas somam e devolucoes abatem consumo liquido |
...
| Relatorios basicos | Relatorio diario, por caixa, produtos mais vendidos, fiados, estoque, consumo de estoque e comandas | `/api/relatorios*` | `Comanda`, `ItemComanda`, `Produto`, `Cliente`, `Pagamento`, `Caixa`, `MovimentoCaixa`, `MovimentoEstoque` | `app/relatorios_test.py` | Implementado | Mais vendidos usa `ItemComanda`; consumo de estoque usa `MovimentoEstoque` |
| Configuracoes | Nome do bar, dias de fiado, consulta e atualizacao de parametros operacionais | `/api/configuracoes` | `ConfiguracaoSistema` | `app/configuracoes_test.py` | Implementado | Cria configuracao padrao quando necessario e nao expoe hash |
| Configuracoes | Governar baixa de estoque por venda via `permitir_estoque_negativo` | API de configuracoes + itens de comanda | `ConfiguracaoSistema`, `Produto`, `ProdutoComposicao`, `MovimentoEstoque` | `app/configuracoes_test.py`, `app/comandas_test.py` | Implementado | Ajuste manual continua rejeitando estoque negativo |
| Acesso / Senha | Acessar sistema com senha simples | `/api/acesso/validar`, `/api/acesso/senha` | `ConfiguracaoSistema` | `app/configuracoes_test.py` | Implementado | Senha unica do MVP, salva apenas como hash |
